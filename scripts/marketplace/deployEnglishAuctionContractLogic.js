...
async function deployEnglishAuctionContractLogic(
  deployerPrivateKey,
  {
  ...
  const code = (
    await readFile(
      process.env.CONTRACTS_DIR + "/english_auction/" + "english_auction_logic.scilla"
    )
  ).toString();
  const init = [
  ...
  console.info("Deploying English Auction Logic Contract...");
  return deployContract(deployerPrivateKey, address, code, init);
}

exports.deployEnglishAuctionContractLogic = deployEnglishAuctionContractLogic;
