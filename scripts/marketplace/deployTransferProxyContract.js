...
async function deployTransferProxyContract(
  deployerPrivateKey,
  ...
  const code = (
    await readFile(
      process.env.CONTRACTS_DIR + "/transfer_proxy/" + "transfer_proxy.scilla"
    )
  ).toString();
  const init = [
  ...
  console.info("Deploying Transfer Proxy Contract...");
  return deployContract(deployerPrivateKey, address, code, init);
}

exports.deployTransferProxyContract = deployTransferProxyContract;
