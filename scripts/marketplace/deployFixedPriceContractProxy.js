...
async function deployFixedPriceContractProxy(
  deployerPrivateKey,
  ...
  const code = (
    await readFile(
      process.env.CONTRACTS_DIR + "/fixed_price/" + "fixed_price_proxy.scilla"
    )
  ).toString();
  const init = [
  ...
  console.info("Deploying Fixed Price Proxy Contract...");
  return deployContract(deployerPrivateKey, address, code, init);
}

exports.deployFixedPriceContractProxy = deployFixedPriceContractProxy;
